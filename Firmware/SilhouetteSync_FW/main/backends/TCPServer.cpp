...
d(d)
{
    ...
    wifi_init_sta();
    xTaskCreate(&tcp_server_task_trampoline, "tcp_server_task", 4096, this, 9, &tcp_server_task_hdl); //launch tcp server task
}
...
{
    TCPServer *local_server = (TCPServer *)arg; //cast to nav switch to pointer

    local_server->tcp_server_task(); //launch nav switch task
}
