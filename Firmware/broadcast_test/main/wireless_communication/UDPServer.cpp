...
UDPServer::UDPServer()
    : sock(-1), packet_stream(sock), s_retry_num(0), connected(false), discovered(false)
{
...
{
    int broad_cast_en = (int) enable;
    if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &broad_cast_en, sizeof(broad_cast_en)) < 0)
